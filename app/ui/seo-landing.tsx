...
export function SeoLanding({ data, media }: { data: SeoLandingData; media?: ReactNode }) {
  return <main className="seo-landing">
    ...
    <section className="seo-answers" id="answers"><div className="container seo-answers-grid"><div><span>DIRECT ANSWERS</span><h2>用户和 AI 搜索真正会问的问题</h2></div><div>{data.questions.map(([q,a])=><article key={q}><h3>{q}</h3><p>{a}</p></article>)}</div></div></section>
