...
function InboxDemo() {
  return <div className="system-demo"><div className="system-demo-content full"><div className="system-demo-bar"><strong>客户工作台</strong><small>AI 辅助 · 草稿需确认</small></div><div className="inbox-demo"><aside><span className="active">新询盘 · 示例 A</span><span>待跟进 · 示例 B</span><span>人工接管 · 示例 C</span></aside><div className="inbox-conversation"><small>来源：Instagram · 产品演示内容</small><p className="incoming">Can you share MOQ and lead time?</p><p className="draft">建议草稿：可以。为确认适用信息，请先告诉我目标市场、规格和预计数量。</p><div><span>意向：待补齐数量</span><span>风险：涉及交期，需确认</span></div></div><section><small>AI 判断依据</small><strong>采购信息未完整</strong><p>已知：产品兴趣、询问 MOQ<br/>未知：市场、规格、数量</p><button type="button">转交销售</button></section></div></div></div>;
}
...
export function ProductEvidence() {
  ...
  return <section className="product-evidence" id="product"><div className="product-evidence-nav" role="tablist" aria-label="产品功能演示">{views.map((view,index)=><button key={view.id} type="button" role="tab" aria-selected={active===view.id} onClick={()=>setActive(view.id)}><span>0{index+1}</span>{view.label}</button>)}</div><div className="product-evidence-stage"><div className="product-evidence-copy"><span>{current.eyebrow}</span><h3>{current.title}</h3><p>{current.text}</p><small>界面演示依据 lingshu-AI 正式产品功能制作；不包含客户名称、客户产品、联系人或经营数据。</small></div><div className={`product-evidence-media is-${active}`}>{active==="create"?<CreateDemo/>:active==="publish"?<PublishDemo/>:<InboxDemo/>}<div className="product-live-mark"><i/> SYSTEM DEMO</div></div></div></section>;
}
